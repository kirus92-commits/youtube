...
Supports rate limiting, resume, and structured output
"""
...
def get_channel_videos(channel_url: str) -> List[Dict]:
    ...
    with yt_dlp.YoutubeDL({'quiet': True, 'extract_flat': 'in_playlist'}) as ydl:
        ...
        channel_info = yt_dlp.YoutubeDL({'quiet': True}).extract_info(
            f"https://www.youtube.com/@{channel_url.split('@')[-1].split('?')[0]}", 
            download=False
        )
...
**아카이빙 일시**: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
**자막 언어**: {language}
**총 길이**: {format_timestamp(segments[-1]["start"] + segments[-1]["duration"]) if segments else "0:00"}
**세그먼트 수**: {len(segments)}
